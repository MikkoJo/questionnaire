...
dojo.declare("agsjs.layers.GoogleMapsLayer", esri.layers.Layer, {
  ...
  _setMap: function (map, layersDiv, index, a, b, c) {
    ...
    this._map = map;
    var div = dojo.create('div', null, layersDiv);
    if (this._options.id) {
      div.id = this.id;
    }
    var style = {
  ...
  _extentChangeHandler: function (extent, delta, levelChange, lod) {
    //console.log("_extentChangeHandler: x: " + delta.x + " y: " + delta.y);
    if (levelChange) {
  ...
  _panHandler: function (extent, delta) {
    //this._setExtent(extent);
    //console.log("_panHandler: x: " + delta.x + " y: " + delta.y);
    // If both delta.x and delta.y are 0, do nothing to prevent unnecessary download of tiles
